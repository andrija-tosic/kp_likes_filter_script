...
with open("graficke.txt", "w", encoding="utf-8") as f:

    min_cena = 20000 #20000
    min_ocena = 100 #1000
    ...
    for k in range(1, br_stranica+1):
        ...
        if greska_za_redom < 3:
            ...
            for i in range(0, br_oglasa): ## parsing cena, pa lajkova
                ...
                if re.search("6G|8G|6 g|6 gb|8 g|8 gb", name, re.IGNORECASE) and not re.search("4GB", name, re.IGNORECASE): ## glavni uslov, posle tek lajkovi
                    ...
                    if likeCount >= min_ocena:
                        ...
                        print(f"Broj lajka je {likeCount}, manje je od minimum od {min_ocena}", file = f)
